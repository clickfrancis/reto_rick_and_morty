...
import '../providers/state/get_unknown_characters_state.dart';

class UnknownCharacters extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Expanded(
          child:
              BlocBuilder<GetUnknownCharactersBloc, GetUnknownCharactersState>(
            builder: (context, state) {
              if (state is GetUnknownCharactersLoadState) {
                return const Center(
                  child: CircularProgressIndicator(),
                );
              } else if (state is GetUnknownCharactersSuccessState) {
                return Padding(
