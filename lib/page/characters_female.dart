...
import '../providers/state/get_female_characters_state.dart';

class FemaleCharacters extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        const Favorites(),
        Expanded(
          child: BlocBuilder<GetFemaleCharactersBloc, GetFemaleCharactersState>(
            builder: (context, state) {
              if (state is GetFemaleCharactersLoadState) {
                return const Center(
                  child: CircularProgressIndicator(),
                );
              } else if (state is GetFemaleCharactersSuccessState) {
                return Padding(
